...
## 自主性纪律（分层：启动用户定，挖掘全自主）

**启动阶段（研究框架决策）由用户定**——三区划分、挖掘起点这类一次性、定错作废的全局决策，必须 `ask_user_question` 让用户拍板，不自行决定。

**挖掘阶段全程自主，禁止中途向用户上交决策**：框架一旦确认，方向选择、组合尝试、证伪、入册、终止判断、test 消费——全部由你按内循环纪律自主执行。
- 「你想怎么继续？」「A 还是 B？」这类挖掘中途问询是被禁止的（2026-08-18 实测：两中途问询打断自主探索）。
- **决策要留痕不留问**：每个方向决策写进 trail 的 attribution/next_hypothesis，用户事后可追溯、可纠正。
